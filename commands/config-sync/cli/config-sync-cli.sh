...
STATE_ROOT="${HOME}/.claude/config-sync"
mkdir -p "$STATE_ROOT"
...
trap 'log_error "config-sync-cli failed on line $LINENO"; exit 1' ERR
...
usage() {
  cat <<'EOF'
Usage: config-sync-cli.sh [options]
...
log_plan_context() {
  log_info "config-sync-cli plan: $PLAN_FILE"
