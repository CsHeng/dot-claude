...
Given a task description and basic characteristics (complexity, data handling, orchestration
requirements), suggest Shell vs Python (or hybrid) in a way that is consistent with the governance
rule-block and easy for agents to act on.
...
Side Effects: May cause agents to load `skill:language-python` or `skill:language-shell` according
to the decision.
