#!/usr/bin/env python3
"""
LLM Governance Command Implementation

This script implements the /llm-governance command with all the optimizations
...
# Import our optimized tools from the same directory
from tool_checker import ToolChecker
from validator import LLMSpecValidator
from dependency_analyzer import DependencyAnalyzer


class OptimizePrompts:
    """Main LLM governance command implementation."""
    ...
    def run(self, target_path: Optional[str] = None, use_all: bool = False) -> int:
        """Main entry point for LLM governance command."""
        print("🚀 Starting LLM Governance with Enhanced Validation")
    ...
    def _discover_all_targets(self) -> List[Path]:
        ...
        # Load directory classification patterns from config.yaml when available
        # Script is in skills/llm-governance/scripts/, so look for config.yaml in same dir
        ...
        if config_file.exists():
            try:
                ...
                # Load patterns from classification section
                for key in ("skills", "commands", "agents", "rules", "core", "output-styles",
                           "governance-rules", "governance-routers", "governance-entrypoints", "governance-styles"):
                    section = classification.get(key)
                    if not section:
                        continue
                ...
                print(f"⚠️  Failed to load classification rules from {config_file}: {e}")
    ...
    def _print_usage(self):
        ...
        print("\nUsage:")
        print("  /llm-governance [path]     # Optimize specific file")
        print("  /llm-governance --all      # Optimize all LLM-facing files")
