...
> **Path Resolution Context**: All relative paths mentioned in this document (such as `rules/**`, `governance/**`, `skills/**`, `agents/**`) are resolved relative to `~/.claude/` (the user's home directory), **not** relative to the current project's `.claude/` directory (if any).
>
> **Examples**:
> - `rules/98-communication-protocol.md` → `~/.claude/rules/98-communication-protocol.md`
> - `governance/routers/**` → `~/.claude/governance/routers/**`
> - `skills/llm-governance/scripts/config.yaml` → `~/.claude/skills/llm-governance/scripts/config.yaml`
